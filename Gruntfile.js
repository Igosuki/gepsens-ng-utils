...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    meta: {
      modules: 'angular.module("gepsens", [<%= srcModules %>]);',
      tplmodules: 'angular.module("gepsens.tpls", [<%= tplModules %>]);',
      all: 'angular.module("gepsens", ["gepsens.tpls", <%= srcModules %>]);'
    },
    delta: {
      html: {
        files: ['template/**/*.html'],
  ...
  //Common gepsens module containing all modules for src and templates
  ...
  function findModule(name) {
    ...
    var module = {
      name: name,
      moduleName: enquote('gepsens.' + name),
  ...
  function dependenciesForModule(name) {
    ...
    .forEach(function(contents) {
      ...
      dependencies.split(',').forEach(function(dep) {
        if (dep.indexOf('gepsens.') > -1) {
          var depName = dep.trim().replace('gepsens.','').replace(/['"]/g,'');
